...
class StudentController {
  ...
  async index(req, res) {
    return res.send((await Students.findAll({
  ...
  async indexStudentsById(req, res) {
    ...
    return res.send((await Students.findOne({
      where: {
        id
      }
    })));
  ...
  async put(req, res) {
    ...
    try {
      ...
      if ((await Users.findOne({
        ...
        return res.json({
          status: "error",
          msg: "Não foi possível alterar o aluno, pois o email inserido já está sendo usado"
        });
      }
