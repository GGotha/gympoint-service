...
class MatrículaController {
  async store(req, res) {
    const schema = Yup.object().shape({
      ...
      plan_id: Yup.number().integer().positive().required(),
  ...
  async delete(req, res) {
    ...
    try {
      ...
      const findForDelete = await Matrículas.destroy({
        where: {
          id
        }
      });
  ...
  async put(req, res) {
    ...
    try {
      const findForUpdate = await Matrículas.findOne({
        where: {
          id
        }
      });
  ...
  async indexMatriculasById(req, res) {
    const id = req.params.id;
    return res.send((await Matrículas.findOne({
      where: {
        id
      },
      include: [{
